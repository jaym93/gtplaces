			  	...
			  	tx.executeSql('CREATE TABLE IF NOT EXISTS buildings (id, name, address, image_url, latitude, longitude, tags)', [], function(tx, results) {
					...
					tx.executeSql("SELECT COUNT(*) as recordCount FROM buildings", [], function(tx, results) {
						...
						if (recordCount == 0) {
							...
							$.getJSON("gtplaces/buildingData.json", insertJSONDataIntoDB);
						}
						else {
	...
	db.transaction(function (tx) {
		tx.executeSql('CREATE TABLE IF NOT EXISTS buildingsTags (buildingid, tag_id, tag)', [], function (tx, results) {
			tx.executeSql("DELETE FROM buildingsTags", [], function(tx, results) {
				...
				// 				startDate = Date.now();
				$.getJSON("gtplaces/gtplaces.php", { "id": "tnames" }, insertTagsDataIntoDB);
			});
		});
	});
}
...
function showPlaceInfoPage(placeID) {
	...
	$("#portal_header #left a img").attr("src", "gtplaces/img/back.png");
...
function confirmFlagTag() {
    ...
    $("#confirmFlagPopup #btnFlag").click(function() {
		tag = "'" + tag + "'";
		console.log(tag);
        $.get("gtplaces/gtplaces.php", { "id": "flag", "bid": currentPlaceID, "tag": tag, "user": PORTAL_CLIENT.getUsername() }, function(data) {
...
function doJSONStuff() {
	...
	// 			{
	// 
	// 				x += "http://"+window.location.hostname+"/gtplaces/gtplaces.php?id=row&bid=" + buildingJSONData[i]["id"] + "\n";
	// 			}
	// 			
	// 			console.log(x);
}
...
function addTag() {
	if ($("#tag_input").is(":visible")) {
		$("#tag_input").hide();
		$("#add_tag_link img").attr("src", "gtplaces/img/button_add_up.png")
	}
	else {
		$("#tag_input").show();
		$("#add_tag_link img").attr("src", "gtplaces/img/button_add_down.png")
	}
}
...
function saveTag() {
	...
	if (newTag.length != 0) {
		...
		$.getJSON("gtplaces/gtplaces.php", { "id": "add_tag", "bid": currentPlaceID, "tag": newTag, "user": PORTAL_CLIENT.getUsername() }, function(data) {
